...
class AIService:
    ...
    def analyze_medical_question(self, question):
        """Analyze a general medical question using Gemini AI with fallback system"""
        try:
            # Add some debugging
            print(f"🔍 Analyzing medical question: {question[:50]}...")
            
            prompt = f"""
            You are a medical AI assistant providing educational information. Please answer the following medical question with accurate, evidence-based information.
            ...
            - If discussing medications, include general warnings about consulting doctors
            
            Question: {question}

            Please provide a comprehensive, educational response while emphasizing that this is not medical advice and patients should consult with qualified healthcare professionals for medical decisions.
            """
            ...
            if response.text:
                return {
                    ...
                    'sources': []  # You can add PubMed sources here if needed
                }
            else:
            ...
            error_message = str(e)
            print(f"❌ Error in medical question analysis: {error_message}")
            
            # Handle specific quota exceeded error
            if "429" in error_message and "quota" in error_message.lower():
                return {
                    'analysis': self._get_quota_exceeded_message(),
                    'sources': []
                }
    ...
    def _get_quota_exceeded_message(self):
        """Return a helpful message when API quota is exceeded"""
        return """
**🚫 API Usage Limit Reached**

I apologize, but we've reached the daily limit for AI-powered responses (50 requests per day on the free tier).

**📝 What this means:**
- The Gemini AI service has a daily quota limit
- This resets every 24 hours
- All previous conversations are still available

**🔄 What you can do:**
1. **Wait for Reset**: The quota will reset in a few hours
2. **Use Basic Responses**: I can still provide basic medical information
3. **Check Back Tomorrow**: Full AI features will be available again

**💡 For Your Question:**
While I can't provide a detailed AI-generated response right now, I recommend:
- Consulting with a healthcare professional for medical advice
- Checking reputable medical websites like WebMD, Mayo Clinic, or MedlinePlus
- Contacting your doctor if you have specific health concerns

**🔒 Your Privacy:**
- Your questions are still processed securely
- No personal health information is stored
- All conversations remain private

**Thank you for understanding!** This helps us keep the service free and available for everyone.

*💡 Tip: The service typically resets around midnight UTC (Universal Coordinated Time).*
        """.strip()
    ...
    def _get_fallback_response(self, question):
        ...
        if any(word in question_lower for word in ['diabetes', 'blood sugar', 'glucose']):
            return {
                'analysis': """
**About Diabetes:**
...
- Lifestyle changes can help manage Type 2 diabetes
- Regular monitoring of blood sugar is essential

**⚠️ Medical Disclaimer:** This is general educational information only. Please consult with a healthcare professional for proper diagnosis, treatment, and medical advice specific to your situation.

**🔄 AI Service Note:** Currently using basic responses due to API limitations. Full AI-powered analysis will be available when the service quota resets.
                """.strip(),
                'sources': ['General Medical Knowledge']
            }
        elif any(word in question_lower for word in ['blood pressure', 'hypertension', 'bp']):
            return {
                'analysis': """
...
**🔄 AI Service Note:** Currently using basic responses due to API limitations. Full AI-powered analysis will be available when the service quota resets.
        ...
        elif any(word in question_lower for word in ['heart', 'cardiac', 'chest pain']):
            return {
                'analysis': """
**About Heart Health:**

The heart is a vital organ that pumps blood throughout your body.
...
**🚨 Emergency:** If you experience severe chest pain, shortness of breath, or other concerning symptoms, seek immediate medical attention or call emergency services.
...
**🔄 Service Status:**
Currently, our AI-powered detailed analysis is temporarily unavailable due to daily usage limits. However, I can still provide some general guidance:

**📚 General Medical Advice:**
- Always consult with qualified healthcare professionals for medical concerns
- Keep track of your symptoms and their timing
- Maintain a healthy lifestyle with proper diet and exercise
- Follow prescribed treatments and medications as directed
...
**⚠️ Important:** This is general educational information only. For specific medical advice, diagnosis, or treatment, please consult with a qualified healthcare professional.

**🔄 AI Service Note:** Full AI-powered analysis will be available when the service quota resets (typically within 24 hours).
                """.strip(),
                'sources': ['General Medical Knowledge']
            }
