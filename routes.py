...
def ask_medical_question():
    """Handle medical assistant questions"""
    try:
        ...
        # Use the AI service to get medical information
        response = ai_service.analyze_medical_question(question)
        
        return jsonify({
            'answer': response.get('analysis', 'I apologize, but I could not generate a response at this time.'),
            'sources': response.get('sources', [])
        })
        
    except Exception as e:
        print(f"Error in medical assistant: {str(e)}")  # Changed from logger to print
...
@app.route('/medicine-info')
@login_required
def medicine_info():
    """Medicine information lookup page with voice features"""
    return render_template('medicine_info.html')

@app.route('/voice-test')
@login_required
def voice_test():
    """Multi-language TTS test page"""
    return render_template('voice_test.html')

@app.route('/api/translate', methods=['POST'])
@login_required
def translate_text():
    """Translate text for TTS in different languages"""
    try:
        data = request.get_json()
        text = data.get('text', '')
        ...
        if target_language == 'en':
            ...
            # For now, we'll provide language-appropriate medical responses
            # This is a placeholder that could be replaced with actual translation service
            
            # Common medical terms and phrases that should work with native TTS
            medical_keywords = ['symptoms', 'treatment', 'medicine', 'doctor', 'health', 'disease', 'pain', 'fever']
            
            if target_language == 'hi':
                # Keep original text but optimize for Hindi TTS
                translated_text = text
                if any(keyword in text.lower() for keyword in medical_keywords):
                    # Add Hindi context for medical terms
                    translated_text = f"स्वास्थ्य जानकारी: {text}"
            elif target_language == 'ta':
                # Keep original text but optimize for Tamil TTS  
                translated_text = text
                if any(keyword in text.lower() for keyword in medical_keywords):
                    # Add Tamil context for medical terms
                    translated_text = f"மருத்துவ தகவல்: {text}"
            elif target_language == 'te':
                # Keep original text but optimize for Telugu TTS
                translated_text = text
                if any(keyword in text.lower() for keyword in medical_keywords):
                    # Add Telugu context for medical terms
                    translated_text = f"వైద్య సమాచారం: {text}"
            else:
                translated_text = text
        ...
        return jsonify({
            ...
            'language_name': language_codes.get(target_language, "English"),
            'message': f'Text prepared for {language_codes.get(target_language, "English")} TTS'
        })
        
    except Exception as e:
        print(f"Translation error: {e}")
        return jsonify({
            'success': True,  # Return success with original text as fallback
            'original_text': data.get('text', ''),
            'translated_text': data.get('text', ''),
            'target_language': target_language,
            'message': 'Using original text (translation service unavailable)'
        })

@app.route('/api/service-status', methods=['GET'])
@login_required
def get_service_status():
    """Get current service status and quota information"""
    try:
        # You can add actual quota tracking here
        return jsonify({
            'ai_service': 'limited',  # Could be: 'active', 'limited', 'unavailable'
            'quota_info': {
                'daily_limit': 50,
                'reset_info': 'Service quota resets every 24 hours',
                'current_time': datetime.utcnow().isoformat(),
                'fallback_active': True
            },
            'message': 'AI service is currently using fallback responses due to quota limits. Full service will restore when quota resets.'
        })
    except Exception as e:
        return jsonify({
            'error': str(e)
